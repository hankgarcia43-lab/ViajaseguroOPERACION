...
const POINTS: Record<string, Point> = {
  ...
  'benito juarez - centro medico (metro)': { lat: 19.4065, lng: -99.1552 },
...
function normalize(value: string) {
  return value.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/ñ/g, 'n').trim();
}
...
function findPoint(value?: string | null) {
  ...
  return Object.entries(POINTS).find(([key]) => normalized.includes(key) || key.includes(normalized))?.[1] ?? null;
}
...
function looksLikeCdmx(value?: string | null) {
  return /metro|cetram|hospital|cuauhtemoc|gustavo|venustiano|benito|tlalpan|coyoacan|iztapalapa|miguel hidalgo|azcapotzalco|cdmx/.test(normalize(value ?? ''));
}

export function estimateRouteDistanceKm(origin?: string | null, destination?: string | null) {
  const originPoint = findPoint(origin);
  const destinationPoint = findPoint(destination);
  if (originPoint && destinationPoint) {
    return Math.max(1, Math.round(haversineKm(originPoint, destinationPoint) * ROAD_FACTOR * 10) / 10);
  }

  const originCdmx = looksLikeCdmx(origin);
  const destinationCdmx = looksLikeCdmx(destination);
